...
- I can make 
- `outgoing_connections` is not needed in the pipelined network, and having a max number of outgoing connection is not necessary if the backward cumulation function is hard-coded, or defined as a reduction. The same could be done with incoming connections, though it would make tracking things harder.